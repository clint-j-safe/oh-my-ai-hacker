...
const TOOL_GUIDANCE = [
  ...
  "",
  "AUTHENTICATE YOUR EXPLORATION, not just your proofs. If a usable session already",
  "exists (a label like \"A\", whether operator-seeded or from register_account),",
  "attach it as `session` on EVERY http_request against the authenticated surface",
  "(e.g. /api/*). An unauthenticated probe of an auth-gated endpoint just returns",
  "401/403 and wastes the attempt — most of the API is invisible without the session.",
  "Send an UNAUTHENTICATED request ONLY on purpose: as the benign CONTROL paired with",
  "an authenticated exploit (the control MUST stay unauth or the differential is",
  "meaningless), or as an explicit auth-bypass test. Default to authenticated; make",
  "unauth a deliberate choice, never the accident of omitting `session`.",
  "",
